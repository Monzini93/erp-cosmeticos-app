...
// A importação estática do jsPDF foi removida para tentar uma abordagem dinâmica.
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          className="flex items-center bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition-colors duration-200"
        >
            ...
            {clientes.map((c) => (
              <tr key={c.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-4">
                  <button type="button" onClick={() => handleOpenModal(c)} className="text-teal-600 hover:text-teal-800">
                    <Edit className="w-5 h-5" />
                  </button>
                  <button type="button" onClick={() => handleDelete(c.id)} className="text-red-600 hover:text-red-800">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {materias.map((m) => (
              <tr key={m.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-4">
                  <button type="button" onClick={() => handleOpenModal(m)} className="text-teal-600 hover:text-teal-800">
                    <Edit className="w-5 h-5" />
                  </button>
                  <button type="button" onClick={() => handleDelete(m.id)} className="text-red-600 hover:text-red-800">
                    <Trash2 className="w-5 h-5" />
                  </button>
                </td>
              </tr>
...
function OrdemServicoFormModal({ onClose }) {
  ...
  const generatePDF = async (ordemData) => {
    // Importação dinâmica robusta para diferentes builds do jspdf
    const mod = await import('jspdf');
    const JsPDF = mod.jsPDF || mod.default; // cobre ESM e CJS
    const docPdf = new JsPDF();

    const dataDoc =
      ordemData.createdAt?.seconds
        ? new Date(ordemData.createdAt.seconds * 1000)
        : typeof ordemData.createdAt?.toDate === 'function'
        ? ordemData.createdAt.toDate()
        : new Date();
  ...
  const handleSubmit = async (e) => {
    ...
    // Gerar PDF (garantindo compat c/ createdAt.seconds)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white rounded-lg p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            type="text"
            placeholder="Nome do Produto Final (ex: Shampoo Hidratante)"
            value={produtoFinal}
          ...
          {ingredientes.map((ing, index) => {
            ...
            return (
              <div key={index} className="flex items-center space-x-2 p-2 border rounded">
                <select
                  value={ing.materiaId}
                  onChange={(e) => handleIngredienteChange(index, 'materiaId', e.target.value)}
                ...
                {materiaSelecionada && (
                  <span className="w-1/4 text-gray-600">
                    Disp: {materiaSelecionada.estoque} {materiaSelecionada.unidade}
                  </span>
                )}
              </div>
            );
          ...
          <button type="button" onClick={addIngrediente} className="text-teal-600 font-semibold">
